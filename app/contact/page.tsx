...
export const metadata = {
  title: 'Contact Sales | CyfroSec',
  description:
    'Talk to the CyfroSec sales team. Tell us about your environment and we will map the fastest path to measurable risk reduction.',
}
  ...
  return (
    <PublicPageShell>
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-12 pt-16 lg:px-6 lg:pt-20 3xl:max-w-[1700px] 3xl:px-8 3xl:pb-16 3xl:pt-24 4xl:max-w-[2000px] bg-white dark:bg-surface-900">
        ...
        />
        <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] cy-text-brand 3xl:text-[13px]">Contact</p>
        <h1
        ...
        >
          Plan your rollout with CyfroSec experts.
        </h1>
        <p className="mt-4 max-w-xl text-base leading-relaxed cy-text-secondary 3xl:max-w-2xl 3xl:text-lg 3xl:leading-8">
          Share your environment, goals, and deployment model. We will help map the fastest path from first
          scan to measurable risk reduction.
        </p>
      </section>
          ...
          <div className="grid gap-10 lg:grid-cols-12 3xl:gap-14">
            ...
            <aside className="lg:col-span-4 lg:order-2 space-y-8 3xl:space-y-10">
              ...
              <div className="space-y-3">
                <p className="mb-4 text-xs font-semibold uppercase tracking-[0.16em] cy-text-muted 3xl:text-[13px]">Other ways to reach us</p>
                {[
                  { icon: Mail, label: 'Email', value: 'sales@cyfrosec.com' },
                ].map((item) => (
                  <div key={item.label} className="flex items-center gap-3 rounded-xl border cy-border-default p-3 cy-bg-muted 3xl:gap-4 3xl:p-4">
                    <div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-primary-500/10 3xl:size-10">
                      <item.icon className="size-4 text-primary-400 3xl:size-[18px]" />
                    </div>
                    <div>
                      <p className="text-xs cy-text-muted 3xl:text-[13px]">{item.label}</p>
                      <p className="text-sm font-medium cy-text-primary 3xl:text-[15px]">{item.value}</p>
                    </div>
                  </div>
                ))}
              </div>
