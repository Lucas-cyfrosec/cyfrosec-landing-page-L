    ...
    <PublicPageShell>
      ...
      <section className="bg-white dark:bg-surface-900 py-20 px-4">
        <div className="mx-auto max-w-5xl">
          <p dir={isAr ? 'rtl' : undefined} className="text-xs font-semibold uppercase tracking-[0.16em] text-primary-400 mb-3">{labels.whatYouGetLabel}</p>
          <h2 dir={isAr ? 'rtl' : undefined} className="text-2xl font-bold cy-text-primary sm:text-3xl mb-10" style={{ fontFamily: HEADING_FONT }}>{pg.outputsTitle}</h2>
            ...
            {pg.outputs.map((output, i) => {
              ...
              return (
                <div key={output.title} className="rounded-2xl border cy-border-default cy-bg-muted p-6">
                  ...
                  <h3 dir={isAr ? 'rtl' : undefined} className="mb-2 text-lg font-bold cy-text-primary" style={{ fontFamily: HEADING_FONT }}>{output.title}</h3>
                  <p dir={isAr ? 'rtl' : undefined} className="text-sm cy-text-secondary leading-relaxed mb-4">{output.description}</p>
                  <ul className="space-y-2">
                    {output.details.map((d) => (
                      <li key={d} className="flex items-start gap-2 text-sm cy-text-secondary">
                        ...
                        <span dir={isAr ? 'rtl' : undefined}>{d}</span>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section className="bg-surface-50 dark:bg-surface-950 py-20 px-4">
        <div className="mx-auto max-w-4xl">
          <h2 dir={isAr ? 'rtl' : undefined} className="text-2xl font-bold cy-text-primary sm:text-3xl mb-4" style={{ fontFamily: HEADING_FONT }}>{pg.riskCardTitle}</h2>
          <p dir={isAr ? 'rtl' : undefined} className="text-sm cy-text-secondary mb-8 max-w-2xl">{pg.riskCardSubtitle}</p>
          <div className="divide-y cy-border-default rounded-2xl border cy-border-default overflow-hidden">
            {pg.riskCardFields.map((row, i) => (
              <div key={row.field} className={`flex gap-4 p-4 ${i % 2 === 0 ? 'cy-bg-elevated' : 'cy-bg-muted'}`}>
                <span dir={isAr ? 'rtl' : undefined} className="w-48 shrink-0 text-sm font-semibold cy-text-primary">{row.field}</span>
                <span dir={isAr ? 'rtl' : undefined} className="text-sm cy-text-secondary">{row.desc}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white dark:bg-surface-900 py-20 px-4">
        <div className="mx-auto max-w-5xl">
          <p dir={isAr ? 'rtl' : undefined} className="text-xs font-semibold uppercase tracking-[0.16em] text-primary-400 mb-3">{labels.sourceTypesLabel}</p>
          <h2 dir={isAr ? 'rtl' : undefined} className="text-2xl font-bold cy-text-primary sm:text-3xl mb-10" style={{ fontFamily: HEADING_FONT }}>{pg.sourcesTitle}</h2>
            ...
            {pg.sources.map((s, i) => {
              ...
              return (
                <div key={s.type} className="rounded-2xl border cy-border-default cy-bg-muted p-5">
                  ...
                  <p dir={isAr ? 'rtl' : undefined} className="text-sm cy-text-secondary leading-relaxed">{s.description}</p>
                </div>
              )
            })}
          </div>
