            ...
            return (
              <div key={feature.title} className="p-4 sm:p-6 bg-white dark:bg-surface-800 rounded-xl border border-surface-200 dark:border-surface-700 hover:border-primary-300 dark:hover:border-primary-600 transition-colors">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h3 dir={isAr ? 'rtl' : undefined} className="font-semibold text-surface-900 dark:text-surface-50">{feature.title}</h3>
                </div>
                <p dir={isAr ? 'rtl' : undefined} className="text-sm text-surface-600 dark:text-surface-400 leading-relaxed">{feature.description}</p>
              </div>
            );
          })}
        </div>
