...
export const BentoGrid = ({ children, className }: BentoGridProps) => (
  <div className={cn('grid w-full auto-rows-auto grid-cols-1 lg:auto-rows-[18rem] lg:grid-cols-3 gap-5', className)}>
    {children}
  </div>
);
...
export const BentoCard = ({
  ...
  return (
    <article
      className={cn(
        'group relative col-span-1 lg:col-span-3 flex flex-col overflow-hidden rounded-2xl',
      ...
      onMouseEnter={e => {
        (e.currentTarget as HTMLElement).style.boxShadow = `0 0 28px -4px ${accentGlow}, 0 8px 40px -8px rgba(0,0,0,0.65)`;
        (e.currentTarget as HTMLElement).style.borderColor = `rgb(${accentRgb} / 0.55)`;
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLElement).style.boxShadow = `0 0 0 0 transparent, 0 4px 32px -8px rgba(0,0,0,0.55)`;
    ...
    >
      ...
      <div
        ...
        style={{ background: `radial-gradient(ellipse at 80% 0%, ${accentTint} 0%, transparent 60%)` }}
      />
      ...
      <div className="relative z-10 flex h-full flex-col p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            ...
            style={{ borderColor: accentBorder, backgroundColor: accentTint, color: accentStrong }}
          >
            {eyebrow}
          </span>
        </div>
        ...
        {/* Footer */}
        <div
          className="mt-4 flex flex-wrap items-start justify-between gap-3 border-t pt-3"
          style={{ borderColor: `rgb(${accentRgb} / 0.14)` }}
        >
          <div
            className="inline-flex max-w-full flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border px-3 py-1.5"
            style={{ borderColor: accentBorder, background: accentTint }}
          >
          ...
          <a
            href={href}
            className="inline-flex shrink-0 items-center gap-1.5 text-[0.8rem] font-semibold text-surface-500 transition-colors duration-200 hover:text-primary-500 dark:text-surface-400 dark:hover:text-white dark:group-hover:text-surface-200"
          >
            {cta}
